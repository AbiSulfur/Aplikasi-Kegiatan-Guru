...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama_lengkap) || empty($username) || empty($password)) {
        ...
        $check_user = mysqli_prepare($koneksi, "SELECT id_user FROM users WHERE username = ?");
        ...
        if (mysqli_num_rows($result_check) > 0) {
            ...
            $stmt = mysqli_prepare($koneksi, "INSERT INTO users (nama_lengkap, username, password, role, status, created_at) VALUES (?, ?, ?, 'guru', 'approved', NOW())");
            mysqli_stmt_bind_param($stmt, 'sss', $nama_lengkap, $username, $hashed_password);
            
            if (mysqli_stmt_execute($stmt)) {
                header("Location: guru.php?msg=" . urlencode("Guru berhasil ditambahkan!"));
                exit();
            } else {
                $error = 'Gagal menambahkan guru: ' . mysqli_stmt_error($stmt);
            }
        }
    }
}
...
<head>
  ...
  <title>Tambah Guru - Aplikasi Kegiatan Guru</title>
...
<main class="main-content">
  <div class="d-flex justify-content-between align-items-center mb-4">
    <div>
      <h2 class="fw-bold mb-1">Tambah Guru Baru</h2>
